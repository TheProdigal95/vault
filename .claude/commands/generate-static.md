...
5. Upload reference images to fal.ai storage. Standard mode uses spec card + style card + product photo. Creative-hero handoff uses approved hero + spec card + product photo when fidelity matters; otherwise approved hero + spec card + style card:
   ...
   node .claude/tools/fal-ai/upload.js --files "[Brand]/00 Context/Brand Spec Card.png" "[Brand]/00 Context/Visual Style Card.png" "[product_photo_path]"

   # Creative-hero handoff with product
   node .claude/tools/fal-ai/upload.js --files "[approved_hero_path]" "[Brand]/00 Context/Brand Spec Card.png" "[product_photo_path]"
   ```
...
3. Use `.claude/tools/fal-ai/generate-video.js` for the Veo 3.1 API call
