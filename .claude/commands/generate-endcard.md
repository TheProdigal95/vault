...
**Tools:**
- `node .claude/tools/endcard-generator/generate.js` — Template-based composition generator
- `node .claude/tools/endcard-generator/ref-to-composition.mjs` — Reference image → composition (uses DESIGN.md)
- `node .claude/tools/endcard-generator/design-tokens.js` — DESIGN.md parser / CSS custom property bridge
- `node .claude/tools/endcard-generator/extract-design.mjs` — Extract/update design tokens from reference images
...
4. Run the generator:
   ```bash
   node .claude/tools/endcard-generator/generate.js \
     --type [type] \
     --palette [palette] \
...
1. Locate the brand's DESIGN.md at `[Brand]/00 Context/DESIGN.md`.
   - If no DESIGN.md exists, offer to generate one from reference images:
     ```bash
     node .claude/tools/endcard-generator/extract-design.mjs \
...
2. Run the single-command orchestrator:
   ```bash
   node .claude/tools/endcard-generator/ref-to-composition.mjs \
     [ref.png] \
...
node .claude/tools/gemini-api/analyze-endcard.mjs [ref.png] --brand [Brand] --output extraction.json

# 2. Generate HTML from extraction data

# 3. Compare against reference
node .claude/tools/endcard-generator/compare-visual.mjs --ref [ref.png] --html [project/index.html]
...
```bash
node .claude/tools/endcard-generator/generate-bg.js \
  --image [ref.png] \
...
```bash
node .claude/tools/endcard-generator/process-logo.js \
...
```bash
node .claude/tools/endcard-generator/export-ae.js \
  --project [project-dir] \
...
node .claude/tools/endcard-generator/design-tokens.js [DESIGN.md] --summary

# CSS custom properties for a palette
node .claude/tools/endcard-generator/design-tokens.js [DESIGN.md] --palette light

# Export all palettes as palettes.json-compatible JSON
node .claude/tools/endcard-generator/design-tokens.js [DESIGN.md] --all-palettes

# Typography utility classes
node .claude/tools/endcard-generator/design-tokens.js [DESIGN.md] --typography
```
