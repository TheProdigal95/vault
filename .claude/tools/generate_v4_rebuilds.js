...
const animations = [
  {
    ...
    label: '# EC-M3 v4 — Sleep Stat (True 1:1 Scale)',
...
<html lang="en">
  ...
  <body>
    <div id="root" data-composition-id="main" data-start="0" data-duration="15" data-width="1080" data-height="1920">
      ...
      <div class="badges" id="badges-row">
        <div class="badge" id="badge1"><span class="badge-title">Magnesium</span></div>
        <div class="badge" id="badge2"><span class="badge-title">Saffron 30MG</span></div>
        <div class="badge" id="badge3"><span class="badge-title">Active B Vitamins</span></div>
      </div>
    </div>
    <script>
  ...
  },
  {
    ...
    label: '# EC-G4 v4 — $2.61 A Day (True 1:1 Scale)',
...
<html lang="en">
  ...
  <body>
    <div id="root" data-composition-id="main" data-start="0" data-duration="15" data-width="1080" data-height="1920">
      ...
      <div class="stat" id="stat">$2.61</div>
  ...
  },
  {
    ...
    label: '# EC-O2 v4 — Save 30% (True 1:1 Scale)',
...
for (const anim of animations) {
  const v4Dir = `IM8/00 Assets/Animations/${anim.name}-v4`;
  ...
  for (const file of filesToCopy) {
    ...
    const destPath = path.join(v4Dir, file);
  ...
  fs.writeFileSync(path.join(v4Dir, 'index.html'), anim.html);

  // 4. Create package.json and hyperframes.json
  fs.writeFileSync(path.join(v4Dir, 'package.json'), JSON.stringify({
    name: `${anim.name}-v4`,
    private: true,
    type: "module",
    scripts: {
  ...
  fs.writeFileSync(path.join(v4Dir, 'hyperframes.json'), JSON.stringify({
  ...
  // 5. Render
  console.log(`Rendering ${anim.name}-v4...`);
  try {
    execSync(`cd "${v4Dir}" && npm run render`, { stdio: 'inherit' });
  } catch(e) {
    console.error(`Failed to render ${anim.name}-v4`);
  }

  // 6. Find generated video
  const renderDir = path.join(v4Dir, 'renders');
  if (fs.existsSync(renderDir)) {
    ...
    if (renders.length > 0) {
      ...
      newNodes.push({"id": `${anim.name}-v4-label`, "type": "text", "text": anim.label, "x": startX, "y": 7800, "width": 600, "height": 40, "color": "4"});
      newNodes.push({"id": `${anim.name}-v4-video`, "type": "file", "file": videoPath, "x": startX, "y": 7850, "width": 400, "height": 711});
...
if (fs.existsSync(canvasPath)) {
  ...
  console.log('Canvas updated with new v4 animations.');
}