...
**First-time setup:**
```bash
cd .claude/tools/ad-library && npm install
```
...
```bash
node .claude/tools/ad-library/batch.js --file /path/to/brands.txt
```

Optional: specify output directory:
```bash
node .claude/tools/ad-library/batch.js --file /path/to/brands.txt --output /path/to/data
```
...
```bash
node .claude/tools/ad-library/batch.js --file /path/to/brands.txt --scrape-only
node .claude/tools/ad-library/batch.js --file /path/to/brands.txt --no-media
```
...
```bash
# Scrape
node .claude/tools/ad-library/scrape.js --brand "BrandName" --url "AD_LIBRARY_URL"

# Download media
node .claude/tools/ad-library/download.js --data .claude/tools/ad-library/data/brand-slug

# Analyze with Gemini (full visual analysis — use for deep dives, not for every PVP)
node .claude/tools/ad-library/analyze.js --data .claude/tools/ad-library/data/brand-slug
```
...
**Clean one brand:**
```bash
node .claude/tools/ad-library/cleanup.js --data .claude/tools/ad-library/data/brand-slug
```

**Clean all brands:**
```bash
node .claude/tools/ad-library/cleanup.js --data .claude/tools/ad-library/data --all
```
