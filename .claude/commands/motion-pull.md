...
**Auth:** Motion uses cookie-based browser sessions. The CLI uses the Surf transport which reads the Chrome profile at `~/.claude/tools/grab/motion-profile/` — the same profile grab.js uses. If auth fails, run a one-time Motion login via grab.js first (see `00 Global/Process/Setup.md`).
...
2. Run: `node .claude/tools/grab/grab.js "https://app.motionapp.com"` (or any Motion URL) — this opens the browser, you log in once, and the session is saved to the profile
