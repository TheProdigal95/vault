...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLICKUP_PP_CLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLICKUP_PP_CLI_FEEDBACK_AUTO_SEND")))
...
When ` + "`CLICKUP_PP_CLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLICKUP_PP_CLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
