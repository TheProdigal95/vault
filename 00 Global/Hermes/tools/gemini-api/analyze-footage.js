...
async function analyzeVideo(videoPath) {
  ...
  const uploadResult = await fileManager.uploadFile(videoPath, {
    mimeType: videoPath.endsWith('.mov') ? 'video/quicktime' : 'video/mp4',
  ...
  const result = await model.generateContent([
    ...
    {
      text: `Analyze this Lifeforce brand footage with extreme granularity for high-performance ad editing.
      ...
      - For UI/Screens: Specify exactly what text or metric is being shown or scrolled past.
...
async function main() {
  ...
  if (videoPaths.length === 0) {
    console.error('Usage: node analyze-footage.js <video1> <video2> ...');
    process.exit(1);
  }
