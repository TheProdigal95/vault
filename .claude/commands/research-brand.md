...
```
Or create `.claude/tools/ad-classifier/.env` with `GEMINI_API_KEY=your_key`.
...
2. **Check prerequisites.** Verify `.claude/tools/ad-classifier/.env` exists and contains `GEMINI_API_KEY`. If missing, stop and ask before proceeding.
