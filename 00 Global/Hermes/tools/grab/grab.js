 ...
 *
 * Session persistence: uses a Chrome profile at ~/.reach-digital/motion-profile/
 * Login once, cookies persist across restarts. If session expires, re-run login.
 *
 * Login command (one-time):
 *   agent-browser --session motion --profile ~/.reach-digital/motion-profile --headed open https://projects.motionapp.com/login
...
async function grabMotion(url, outputDir, result) {
  ...
  const MOTION_PROFILE = path.join(HOME, '.reach-digital/motion-profile');
  ...
  const LOGIN_CMD = isWindows
    ? `agent-browser --session motion --profile "%USERPROFILE%\\.reach-digital\\motion-profile" --headed open https://projects.motionapp.com/login`
    : `pkill -f agent-browser 2>/dev/null; sleep 1; agent-browser --session motion --profile ~/.reach-digital/motion-profile --headed open https://projects.motionapp.com/login`;
