...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  clickup-pp-cli auth setup\n  clickup-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export CLICKUP_PP_CLI_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  clickup-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export CLICKUP_PP_CLI_BEARER_AUTH=\"your-token-here\"")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CLICKUP_PP_CLI_BEARER_AUTH") != "" {
				envStillSet = "CLICKUP_PP_CLI_BEARER_AUTH"
			}
