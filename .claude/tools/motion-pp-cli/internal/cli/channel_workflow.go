...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  motion-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  motion-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"creative", "organization"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  motion-pp-cli workflow status

  # Show status as JSON
  motion-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("motion-pp-cli")
			}
