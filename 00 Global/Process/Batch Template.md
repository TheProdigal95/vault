...
9. **Tier 1 — Writer dispatch.** Scripts produced in parallel by script-writer sub-agents (3 scripts each, grouped by structure or persona). Briefs produced in parallel by brief-writer sub-agents (2-3 briefs each, grouped by format or topic). Script-writers run a 19-point concrete checklist (binary pass/fail with evidence) + viewer read (3 read-throughs per script, one per hook, answering 4 narrative-logic questions). Brief-writers spawn `scoring-evaluator` sub-agents for a 7-dimension scoring loop (from `00 Global/Statics Generator/Scoring Agents/`). For a 15-script batch: 5 script-writer agents in parallel. Each writer returns a **structured evidence report** with checklist results + viewer read results + within-group cross-check per [[Critique Methodology - Video]] / [[Critique Methodology - Static]]. Reports missing checklist evidence or viewer read are non-compliant — the orchestrator rejects and re-dispatches.
...
11. **Tier 3 — Main session writes the Batch Critique**: runs `motion-pp-cli highlights --brand [brand]` automatically and surfaces the results in the critique's carry-forward section (what Motion's AI flags to scale vs fix). Then runs the final pass, surfaces patterns, **edits target criteria docs in the same turn for any pattern hitting the 3+ items / 2+ batches promotion threshold**. The `Promote to Global Criteria` section of the critique cites the concrete file + line of each edit made — not proposals, not TODOs. This is the document the strategist reads.
