...
This is the Obsidian vault where all creative strategy work happens for Reach Digital's DTC brand clients. Everything lives in markdown. Claude is an active collaborator — analyzing ads, writing scripts, running research, and producing creative briefs.
...
**Triggers:** "set me up", "this is my first time", "run the setup", "new machine", "onboard me", a slash command fails with "not installed" / "command not found", or the user asks about setup or installing tools.

**When triggered, run this flow:**

1. **Run the check first** to see what's missing:
   ```bash
   bash setup.sh --check
   ```
2. If everything is green, tell the user they're all set.
3. If anything is missing, **ask the user for these values before running setup** (skip any that `--check` already shows as configured):
   - **Gemini API key** — free from https://aistudio.google.com/apikey (needed for video analysis, ad classification)
   - **fal.ai API key** — from https://fal.ai/dashboard/keys (needed for image generation)
   - **Full name** — for ClickUp task assignment
   - **ClickUp user ID** — ask team lead if they don't know it
4. **Run setup with the values pre-filled** (the script accepts env vars to skip interactive prompts):
   ```bash
   GEMINI_API_KEY="<key>" FAL_KEY="<key>" STRAT_NAME="<name>" STRAT_CLICKUP_ID="<id>" bash setup.sh
   ```
   Omit any env var the user skipped or that was already configured.
5. **Walk the user through browser logins** that the script can't automate:
   - Motion login (required for `/motion-pull`): run the `agent-browser` command, tell them to log in, then close it
   - Amazon / Trustpilot logins (on-demand — only mention if the user asks about review scraping)
6. Tell them to **restart Obsidian** (Cmd+Q → reopen) to pick up the new plugins and theme.

**Hard rules:**
- Do not proactively check tool installation on every session — let triggers fire it.
- Never run `bash setup.sh` without the env vars if the user provided API keys — the script would prompt interactively in the terminal and hang waiting for input.
- The setup script is idempotent. Safe to re-run. When in doubt, run `bash setup.sh --check` first.
...
This vault has custom slash commands at `.claude/commands/` (project-level, shared with the repo). When the user's request matches one of these, **invoke it with the Skill tool** instead of trying to do it manually. The commands contain the exact tools, paths, and logic needed.
...
- Pastes URLs from YouTube, TikTok, or direct media links alongside raw notes or analysis intent
- "analyze this ad [url]" / "break down this creative [url]" / "what's this ad doing [url]"
- "here are the top spenders [urls + notes]" / "analyze these top spenders" (for non-Motion URLs)
...
- **Note:** Motion report URLs and Motion pulls route through `/motion-pull`, not `/grab`. Use `/grab` only for non-Motion sources (YouTube, TikTok, competitive refs, ad library links, direct media URLs).
...
### `/motion-pull` — Pull Top Creatives from Motion API

**Invoke this command when the user says anything like:**
- "pull the top spenders from Motion" / "pull [Brand]'s top creatives"
- "run ads analysis for [Brand]" / "let's do the ads analysis for [Brand]"
- "what are the top creatives for [Brand]?" / "pull the Motion report for [Brand]"
- "pull the top 20 for [Brand]" / "get the top spenders"
- Starting a new batch that implies pulling current performance data first

**What it does:** Calls the Motion GraphQL API directly (no CSV, no DOM scrape), stores every pull in a local SQLite history, and enables delta/trend/stale commands on top. Binary at `$HOME/go/bin/motion-pp-cli`. Fully replaces the old CSV + grab.js Motion workflow.

**Hard rules:**
- Always use the absolute path `$HOME/go/bin/motion-pp-cli` — do not rely on bare `motion-pp-cli` in PATH.
- `--brand` auto-resolves workspace ID. `--report-name` auto-resolves report ID. Never ask the user to look up IDs manually.
- Use `--download` to fetch media files — it curls CDN URLs directly in Go, no grab.js needed. Default output dir is `./motion-downloads/`; point it at `[Brand]/02 Ads Analysis/media` for batch work.
- The CLI constructs **Motion share links** from the creative asset ID. Use these as the canonical reference format in scripts, briefs, and analysis docs. CDN URLs are for download/analysis only — never use them as permanent references.
- Use `/grab` only for non-Motion sources (YouTube, TikTok, competitive refs, other URLs).
- If auth fails (401 / login redirect), run a one-time Motion login via grab.js first, then retry.
- **`pull`, `angles`, and `formats` run automatically at batch step 2** — in parallel, before any Gemini analysis or Batch Plan work. Never wait for the strategist to ask for them.
- **`highlights` runs automatically at batch step 11** — before writing the Batch Critique carry-forward section. Never wait for the strategist to ask for it.
- `delta`, `stale`, and `history` are on-demand — run when the strategist asks about changes, trends, or a specific creative's history.
...
**When triggered, read `00 Global/Process/Batch Template.md`** — it contains the three-file template (Working Document, Scripts, Batch Critique), the Concept Validation gate (7-test quality bar + batch-level differentiation rule + conflict resolution hierarchy), the carry-forward rules for subsequent batches (including dead-angle tracking), and the full 12-step batch flow from top spenders analysis through script critique. Also read `00 Global/Process/Top Spenders Analysis Template.md` for the three-lens analysis format (SPEND / CONVERSIONS / RETENTION) used at step 2-3.

**Auto-fire rule:** Concept Validation fires automatically via a sub-agent immediately after the Batch Plan is written — do not wait for the strategist to ask for it.
...
**Hard rules regardless of the detail doc:**
- **Hooks are grounded in account data, not first principles.** Before writing any hooks — in main session or when dispatching a `script-writer` sub-agent — extract two things: (1) the winning hook transcriptions from `[Brand]/01 Ads Analysis/` (exact VO/caption lines from the top-ROAS ads), and (2) verbatim VoC phrases from `[Brand]/00 Context/Reddit Research Sprints/` or equivalent review research. Hooks are built by combining those two sources. Generated-from-scratch hooks that don't mirror actual buyer language are a first-draft failure.
...
- **Visual Direction standard: 5-6 lines minimum** covering b-roll types, caption style, edit pace, color treatment, and format-specific notes. Must trace to visual patterns from the Top Spenders Analysis. Generic descriptions ("aspirational and energetic") fail. For VO + B-roll and mixed formats, the Visual column must specify enough b-roll for ~2-3 second cuts per beat. See Video Script Criteria § Visual & B-Roll Criteria.
...
- **Dispatch model.** Each `script-writer` handles 3 scripts. For a 15-script batch: 5 script-writer agents dispatched in parallel, each writing 3 scripts. After all return, the `critique-orchestrator` runs Tier 2 cross-batch sweeps (including brand compliance, which is batch-level). Main session writes the Batch Critique (Tier 3).
- **3-tier QA workflow applies to batch production.** Writer sub-agents run a 19-point concrete checklist (binary pass/fail with evidence per check) + viewer read (3 read-throughs per script, one per hook variation, answering 4 narrative-logic questions) + within-group cross-check before return. Quality reference rubrics at `00 Global/Video Script Scoring/`. Orchestrator validates evidence reports (rejects reports missing checklist evidence or viewer read) + runs cross-script sweeps + brand compliance sweep. Main session writes the Batch Critique. See `00 Global/Process/Critique Methodology - Video.md`.
...
**CLI:** Use `$HOME/go/bin/clickup-pp-cli` for all loading. Full command reference at `.claude/commands/clickup-load.md`. The CLI handles brand→list routing, strategist detection, task type, and assignees automatically.

```bash
$HOME/go/bin/clickup-pp-cli load "IM8/T003 Scripts.md"         # scripts → Video ads
$HOME/go/bin/clickup-pp-cli load "IM8/T003 Briefs.md"          # briefs → Static ads
$HOME/go/bin/clickup-pp-cli load "IM8/T003 Scripts.md" --dry-run  # preview first
$HOME/go/bin/clickup-pp-cli footage-request --script-file "IM8/T003 Scripts.md" --slug "GLP1" --title "IM8_Greenscreen GLP1"
```

**When triggered, also read `00 Global/Process/ClickUp Integration.md`** — it contains the strategist detection flow, team roster with ClickUp IDs, the Brand → Creative Briefs list table, the fallback rule for resolving the list when a brand isn't in the table, unified brief-and-script loading rules (task name = slug, description = slug-to-Close content), footage request format, and the "waiting on footage" comment pattern.
...
- Resolving a brand's list: the CLI embeds the Brand→List ID table and auto-resolves. For brands not in the table, pass `--list-id`; then add the brand to `brandListIDs` in `pm_load.go` and rebuild.
...
- **Strategists work fast.** They provide raw voice-note-style notes and expect Claude to fill gaps — run Gemini analysis on media, cross-reference patterns from prior brands, surface things they didn't mention. Don't just reformat their notes. Add value.
...
- **Save workflow rules to the system, not memory.** When strategist feedback is a workflow, process, format, or criteria rule that applies across sessions, migrate it to the right file (`CLAUDE.md`, `00 Global/Process/*.md`, `00 Global/Criteria/*.md`) instead of — or in addition to — saving a memory. Memory is for user facts, active project context, external system IDs, and personal preferences (e.g., voice-dictation handling) — not rules the whole system should always know. If you notice an existing memory that duplicates a system rule, propose deleting the memory. Memory doesn't always load; CLAUDE.md and triggered process docs do.
