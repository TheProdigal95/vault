...
**Single URL:**
```bash
node .claude/tools/grab/grab.js "URL_HERE"
```

**Multiple URLs:**
```bash
node .claude/tools/grab/grab.js "URL1" "URL2" "URL3"
```

**Motion report batch mode (Top Spenders workflow):**
```bash
# Download ALL creatives from a Motion report page
node .claude/tools/grab/grab.js "MOTION_REPORT_URL"

# Download only specific ads (matched by name from CSV)
node .claude/tools/grab/grab.js "MOTION_REPORT_URL" --ad-names "Ad Name 1" "Ad Name 2" "Ad Name 3"
```
...
All files go to `.claude/tools/grab/downloads/session-<datetime>/`.
...
```bash
node .claude/tools/gemini-api/gemini-api.js "Please watch the attached video, and convert it into a script with 4 columns. Format as a markdown table with columns: | Timestamp | Visual Action | Voiceover | On-Screen Captions |
...
1. **Motion report URL provided:** Extract ad names from the CSV table, run `node .claude/tools/grab/grab.js "REPORT_URL" --ad-names "Ad 1" "Ad 2" ...`
2. **Individual URLs provided:** Run `node .claude/tools/grab/grab.js "URL1" "URL2" ...`
...
7. **Delete all media after analysis is saved:** `rm -rf .claude/tools/grab/downloads/session-*`
...
Motion cookies persist in a Chrome profile at `~/.claude/tools/grab/motion-profile/` (user's home directory — not shared, per-user auth).
...
2. Kill stale daemons and open headed browser:
   - macOS/Linux: `pkill -f agent-browser 2>/dev/null; sleep 1; agent-browser --session motion --profile ~/.claude/tools/grab/motion-profile --headed open https://projects.motionapp.com/login`
