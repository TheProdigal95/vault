...
import (
	...
	"motion-pp-cli/internal/cliutil"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "motion-pp-cli", "http")
	jar, cookieURL := cookieJarFromConfig(cfg)
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			for k, v := range c.Config.Headers {
				if strings.EqualFold(k, "Cookie") && c.cookieJar != nil {
					continue
				}
				req.Header.Set(k, v)
			}
...
func cookieJarFromConfig(cfg *config.Config) (http.CookieJar, *url.URL) {
	...
	jar.SetCookies(u, parseCookieHeader(rawCookie))
	return jar, u
}
...
func (c *Client) persistCookies() {
	if c == nil || c.Config == nil || c.cookieJar == nil || c.cookieURL == nil || c.Config.Path == "" {
		return
	}
	cookies := c.cookieJar.Cookies(c.cookieURL)
	if len(cookies) == 0 {
		return
	}
	...
	c.Config.Headers["Cookie"] = strings.Join(parts, "; ")
	if err := c.Config.Save(); err != nil {
		fmt.Fprintf(os.Stderr, "warning: could not persist refreshed cookies: %v\n", err)
	}
}
