...
3. **Tier 2 orchestration** — the main session loads the `critique-orchestrator` skill and audits the Scripts file against the Working Document, brand docs, active criteria, and writer evidence. The orchestrator runs category sweeps across the batch first, then item-level re-verify only for items that failed a sweep, and loops fix passes back through `script-writer` until the batch is clean. Top Spenders Analysis is not a required QA input at this stage; use the strategy already distilled into the Working Document unless the strategist asks otherwise or a cited asset/reference must be verified.
4. **Tier 3 critique** — once the orchestrator returns ✅ clean (or ⚠️ flags surfaced), the main session reads the cleaned Scripts file and writes `T[###] Batch Critique.md` using the Output Format below. Any promotion to global criteria is drafted as a concrete proposed rule/edit and requires explicit strategist approval before patching global criteria.
...
The `script-writer` Hermes sub-agent is orchestrated by the active Hermes skill at `~/.hermes/profiles/reach-digital/skills/reach-digital/script-writer/SKILL.md` and the project command contract at `00 Global/Hermes/Commands/script-writer.md`. It produces 3 scripts per invocation and self-audits each before returning. Two-step self-QA per script:
...
- Visual Direction-to-Working Document traceability: verify that Visual Direction patterns (edit pace, color treatment, b-roll types, caption style) trace back to approved Working Document direction or explicit strategist instructions. Writers do not independently mine Top Spenders Analysis at this stage; if the Working Document lacks enough visual direction, surface/fix the Working Document instead of inventing a new visual approach.
