...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOTION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOTION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "motion-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.motion-pp-cli/feedback.jsonl.
When ` + "`MOTION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOTION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  motion-pp-cli feedback list
  motion-pp-cli feedback list --limit 5
  motion-pp-cli feedback list --json`,
