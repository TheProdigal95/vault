...
Resource scoping:
  ...
  motion-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  motion-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  motion-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  motion-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  motion-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/motion-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"creative":     "/api/v2/creative/getAdCommentsByCreativeAssetWeb",
		"organization": "/organization/6214fe5e1ccc60d23897d87b/697a256b66437695f674687d",
	}
