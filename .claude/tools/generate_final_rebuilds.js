...
const animations = [
  {
    ...
    label: '# EC-M3 FINAL — Perfect Typography & Icons',
...
<html lang="en">
  ...
  },
  {
    ...
    label: '# EC-G4 FINAL — 1:1 Condensed Sans (Oswald)',
...
<html lang="en">
  ...
  },
  {
    ...
    label: '# EC-O2 FINAL — 1:1 Wide Sans (Epilogue)',
...
for (const anim of animations) {
  const finalDir = `IM8/00 Assets/Animations/${anim.name}-final`;
  ...
  fs.writeFileSync(path.join(finalDir, 'index.html'), anim.html);

  fs.writeFileSync(path.join(finalDir, 'package.json'), JSON.stringify({
    name: `${anim.name}-final`,
    private: true,
    type: "module",
    scripts: {
  ...
  fs.writeFileSync(path.join(finalDir, 'hyperframes.json'), JSON.stringify({
    "$schema": "https://hyperframes.heygen.com/schema/hyperframes.json",
    "registry": "https://raw.githubusercontent.com/heygen-com/hyperframes/main/registry",
    "paths": { "blocks": "compositions", "components": "compositions/components", "assets": "assets" }
  }, null, 2));

  console.log(`Rendering ${anim.name}-final...`);
  try {
    execSync(`cd "${finalDir}" && npm run render`, { stdio: 'inherit' });
  } catch(e) {
    console.error(`Failed to render ${anim.name}-final`);
  }
  ...
  if (fs.existsSync(renderDir)) {
    ...
    if (renders.length > 0) {
      ...
      newNodes.push({"id": `${anim.name}-final-label`, "type": "text", "text": anim.label, "x": startX, "y": 10200, "width": 600, "height": 40, "color": "4"});
      newNodes.push({"id": `${anim.name}-final-video`, "type": "file", "file": videoPath, "x": startX, "y": 10250, "width": 400, "height": 711});
...
if (fs.existsSync(canvasPath)) {
  ...
  console.log('Canvas updated with final animations.');
}